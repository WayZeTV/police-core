...
AddEventHandler("genius:sendcall", function()
	...
	for i = 1, #xPlayers, 1 do
		local thePlayer = ESX.GetPlayerFromId(xPlayers[i])
		if thePlayer.job.name == 'police' then
			TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], "Central LSPD", nil, ""..name.." demande un agent LSPD au commissariat", "CHAR_CALL911", 3)
		end
	end
end)
