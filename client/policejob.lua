...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{ label = 'Gilet Officier 1', value = 'tenu_hiv'},
	    { label = 'Sac à dos', value = 'bags_wear' },
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		if
			...
			data.current.value == 'bags_wear' or
...
local elements = {
	...
	if Config.EnableArmoryManagement then
		table.insert(elements, {label = ('Déposer une arme saisie'),     value = 'put_weapon'})
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		...
		align    = 'top-left',
		elements = {
			{label = '<span style="color:orange;">Statut de l\'agent', value = 'status'},
			{label = '<span style="color:#00cc00;">Intéraction Citoyen', value = 'citizen_interaction'},
			{label = '<span style="color:#00cc00;">Intéraction Véhicule', value = 'vehicle_interaction'},
			{label = '<span style="color:#669999;">Menu Menottes', value = 'menu_menottes'},
			{label = '<span style="color:#669999;">Donner une ammende', value = 'ammende'},
			{label = '<span style="color:#669999;">Récupérer son équipement', value = 'lspd_equipement'},
			{label = '<span style="color:red;">Demander du renfort', value = 'renfort'}
	}}, function(data, menu)
		...
		elseif data.current.value == 'lspd_equipement' then
			...
			if grade == 'recruit' then
				table.insert(elements, {label = ('Équipement Officier 1'), value = 'equipement_cadet'})
			elseif grade == 'officer' then
				table.insert(elements, {label = ('Équipement Officier 2'), value = 'equipement_officier'})
			elseif grade == 'sergeant' then
				table.insert(elements, {label = ('Équipement Officier 3'), value = 'equipement_sergeant'})
			elseif grade == 'lieutenant' then
				table.insert(elements, {label = ('Équipement Lieutenant'), value = 'equipement_lieutenant'})
			elseif grade == 'boss' then
				table.insert(elements, {label = ('Équipement Capitaine'), value = 'equipement_cmd'})
			end
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'lspd_equipement', {
				...
				elseif action == 'equipement_sergeant' then
					...
					GiveWeaponToPed(PlayerPedId(), "WEAPON_STUNGUN", 255, false, false)
					GiveWeaponToPed(PlayerPedId(), "WEAPON_NIGHTSTICK", 255, false, false)
					GiveWeaponToPed(PlayerPedId(), "WEAPON_FLASHLIGHT", 25, false, false)
					GiveWeaponToPed(PlayerPedId(), "WEAPON_PUMPSHOTGUN", 25, false, false)
...
Citizen.CreateThread(function()
    ...
    ped = CreatePed("PED_TYPE_CIVFEMALE", "s_f_y_cop_01", 433.57, -985.76, 29.70, 87.86, false, true)
...
Citizen.CreateThread(function()
    ...
    ped = CreatePed("PED_TYPE_CIVFEMALE", "s_m_y_cop_01", 462.748, -1003.991, 23.914, 180.23, false, true)
    SetBlockingOfNonTemporaryEvents(ped, true)
    FreezeEntityPosition(ped, true)
    TaskStartScenarioInPlace(ped, "WORLD_HUMAN_COP_IDLES", 0, false)
    SetEntityInvincible(ped, true)
...
Citizen.CreateThread(function()
    ...
    ped = CreatePed("PED_TYPE_CIVFEMALE", "s_m_y_cop_01", 460.061, -1018.016, 31.986, 87.622, false, true)
    SetBlockingOfNonTemporaryEvents(ped, true)
    FreezeEntityPosition(ped, true)
    TaskStartScenarioInPlace(ped, "WORLD_HUMAN_BINOCULARS", 0, false)
    SetEntityInvincible(ped, true)
end)
...
Citizen.CreateThread(function()
    ...
    ped = CreatePed("PED_TYPE_CIVFEMALE", "s_m_y_cop_01", 443.501, -995.0862, 29.689, 269.9, false, true)
    SetBlockingOfNonTemporaryEvents(ped, true)
    FreezeEntityPosition(ped, true)
...
Citizen.CreateThread(function()
    ...
    ped = CreatePed("PED_TYPE_CIVFEMALE", "s_m_y_cop_01", 454.0, -979.9973, 29.689, 87.19, false, true)
    SetBlockingOfNonTemporaryEvents(ped, true)
    FreezeEntityPosition(ped, true)
...
Citizen.CreateThread(function()
    ...
    ped = CreatePed("PED_TYPE_CIVFEMALE", "s_f_y_cop_01", 459.717, -989.2093, 23.914, 272.17, false, true)
    SetBlockingOfNonTemporaryEvents(ped, true)
    FreezeEntityPosition(ped, true)
    TaskStartScenarioInPlace(ped, "WORLD_HUMAN_HIKER", 0, false)
    SetEntityInvincible(ped, true)
end)

-- Plainte

local mainMenu = RageUI.CreateMenu("Accueil de Police", "Que puis-je faire pour vous ?")
local PlainteMenu =  RageUI.CreateSubMenu(mainMenu, "Plainte ", "Décrivez votre plainte")
local RdvMenu = RageUI.CreateSubMenu(mainMenu, "Rendez-vous ", "Prenez un Rendez-vous")
...
Citizen.CreateThread(function()
    while true do
        ...
        RageUI.IsVisible(mainMenu, function()
            RageUI.Button('Demandez un agent de la LSPD', nil, {RightLabel = "→"}, true, {onSelected = function()
                TriggerServerEvent("genius:sendcall") 
                Notification("~r~Central LSPD",nil, "Votre appel à bien été pris en compte", "CHAR_CALL911", 8)

            end});
            RageUI.Button('Déposer une plainte', nil, {RightLabel = "→"}, true, {onSelected = function() 
            ESX.ShowNotification("~b~Merci d'être sérieux dans votre déposition tout abus sera sanctionné")
            end},PlainteMenu);

            RageUI.Button('Prendre un rendez-vous', nil, {RightLabel = "→"}, true, {onSelected = function() 
            ESX.ShowNotification("~b~Cette fonctionalité n'est pas encore disponible, elle le sera prochainement")
            end},RdvMenu);
        ...
        RageUI.IsVisible(PlainteMenu, function()

            RageUI.Button('Indiquez votre ~b~Nom de famille~s~ : ', LastName, {RightLabel = "→"}, true, {onSelected = function() 
                LastName = KeyboardInput("Nom :",nil,20)
            end});

            RageUI.Button('Indiquez votre ~b~Prénom~s~ : ', FirstName, {RightLabel = "→"}, true, {onSelected = function() 
                FirstName = KeyboardInput("Prénom :",nil,20)
            end});

            RageUI.Button('Indiquez votre ~b~Numéro de téléphone~s~ : ', tel, {RightLabel = "→"}, true, {onSelected = function() 
                tel = KeyboardInput("Numéro de téléphone :",nil,350)
            end});

            RageUI.Button('Sujet de votre ~b~Plainte~s~ : ', Subject, {RightLabel = "→"}, true, {onSelected = function() 
                Subject = KeyboardInput("Sujet de votre plainte :",nil,30)
            end});

            RageUI.Button('Décrivez votre ~b~Plainte~s~ : ', Desc, {RightLabel = "→"}, true, {onSelected = function() 
                Desc = KeyboardInput("Décrivez votre plainte:",nil,350)
            end});
            ...
            RageUI.Button('~g~Envoyer ', Desc, {RightLabel = "→"}, cansend, {onSelected = function() 
                RageUI.CloseAll()
                TriggerServerEvent("genius:sendplainte", LastName, FirstName,tel ,Subject, Desc)
                Notification("~r~Central LSPD",nil, "Votre plainte a bien été prise en compte", "CHAR_CALL911", 8)
                reset()
            end});
...
CreateThread(function()
    ...
    for _,c in pairs(Config.ped) do 
        ...
        local Spawnpos = vector3(c.pos.x,c.pos.y,c.pos.z - 1)
        local heading = c.pos.h
        ...
        TaskStartScenarioInPlace(Spawnped, "WORLD_HUMAN_CLIPBOARD", 0, false)
    end
    ...
    while true do
        ...
        for _,c in pairs(Config.ped) do  
        distance = GetDistanceBetweenCoords(pos, vector3(c.pos.x,c.pos.y,c.pos.z - 1), true)

        end
        
        if distance < 3.0 then
            wait = 0
            Visual.Subtitle("Appuyez sur ~b~E~s~ pour parler avec l'~b~agent", 1)
